...
def dls_search(start_state, goal_state, dumpflag, depth_limit=100):
    """
    Depth-limited search algorithm to find a path from start_state to goal_state within a given depth limit.
    Returns a tuple containing nodes_popped, nodes_expanded, nodes_generated, max_frontier_length, depth,
    cost, and steps (a list of actions taken to get from start to goal).
    """
    ...
    while frontier:
        max_frontier_length = max(max_frontier_length, len(frontier))
            ...
            for successor in node.expand():
                if tuple(map(tuple, successor.state)) not in visited:
                    if successor.is_goal(goal_state):
                        ...
                        steps.reverse()
                        return [nodes_popped, nodes_expanded, nodes_generated, max_frontier_length, depth+1, steps]
