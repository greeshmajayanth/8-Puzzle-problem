...
class Node:
    ...
    def expand(self, goal_state):
        ...
        for action in self.get_actions():
            ...
            successors.append(Node(next_state, self, action, 0, h, moved_value))
        return successors
    ...
    def get_heuristic_value(self, state, goal_state):
        """
        Returns the Manhattan distance between this state and the goal state.
        """
        h = 0
        for i in range(3):
            for j in range(3):
...
def greedy_search(start_state, goal_state, dumpflag):
    """
    Greedy search algorithm to find the shortest path from start_state to goal_state.
    ...
    frontier.put((start_node.h, start_node))
    ...
    while not frontier.empty():
        ...
        for successor in node.expand(goal_state):
            if tuple(map(tuple, successor.state)) not in visited:
                ...
                frontier.put((successor.h, successor))
                nodes_generated += 1
